...
if(isset($_POST['add'])) // when click on Update button
{
	...
	if ($stmt = $conn->prepare('INSERT INTO events (event_name, event_desc, event_date, event_location) VALUES (?, ?, ?, ?)')) {
	
	$stmt->bind_param('ssss', $_POST['name'], $_POST['desc'],$_POST['date'], $_POST['location']);
	$stmt->execute();
	echo 'You have successfully added another event';
        ...
        <div class="form-group">
  		...
  		<div class="form-group">
			<div class="input-group">
				...
				<input type="datetime-local" class="form-control" name="date"  placeholder="Date" required="required">
			</div>
        </div>
		<div class="form-group">
			<div class="input-group">
