...
if(isset($_POST['add'])) // when click on Update button
{
	...
	if ($stmt = $conn->prepare('INSERT INTO expense (expense_name, expense_amount, expense_due_date) VALUES (?, ?, ?)')) {
	
	$stmt->bind_param('sss', $_POST['name'], $_POST['amount'],$_POST['due']);
	$stmt->execute();
	echo 'You have successfully added another expense';
...
<div class="signup-form">
    ...
    <form  method="post">
		...
		<p>Please fill in all the columns</p>
		<hr>
		...
		<div class="form-group">
			<div class="input-group">
				...
				<input type="datetime-local" class="form-control" name="due" placeholder="Due Date" required="required">
			</div>
        </div>
