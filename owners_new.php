...
if(isset($_POST['add'])) // when click on Update button
{
	...
	if ($stmt = $conn->prepare('INSERT INTO tennants (tennants_name, tennants_address, tennants_gender, tennant_purchase_date) VALUES (?, ?, ?, ?)')) {
	
	$stmt->bind_param('ssss', $_POST['name'], $_POST['address'],$_POST['gender'], $_POST['pdate']);
	$stmt->execute();
	echo 'You have successfully added another Tennant';
    ...
    <form  method="post">
		...
		<div class="form-group">
			<div class="input-group">
				...
				<input type="datetime-local" class="form-control" name="pdate" placeholder="Prchase Date" required="required">
			</div>
        </div>
